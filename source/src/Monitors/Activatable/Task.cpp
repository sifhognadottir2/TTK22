...
namespace Monitors
{
  ...
  namespace Activatable
  {
    ...
    struct Task: public DUNE::Tasks::Task
    {
      ...
      //! Task arguments
      Arguments m_args; //So we can read from the .ini file
      ...
      Task(const std::string& name, Tasks::Context& ctx):
        ...
        m_counter(0) //Each time the task is called the counter is reset
      {
        ...
        while (!stopping())
        {
          ...
          if (isActive())
          {
            if (m_counter <= m_args.limit){
              war("Received %d Announce messages so far", m_counter);
            }
            else if (m_counter > m_args.limit){
              requestDeactivation();
            }
          }
        }
